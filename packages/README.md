...
Global packages may depend only on global packages. A game-scoped package may
depend on global packages and packages in that same game scope. Run
`npm.cmd run workspace:check` for a focused result; `npm.cmd run lint` runs it
automatically.
